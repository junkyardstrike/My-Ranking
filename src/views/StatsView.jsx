...
export default function StatsView() {
  ...
  const stats = useMemo(() => {
    ...
    allItems.forEach(item => {
      ...
      let unitDuration = (item.duration && Number(item.duration) > 0) ? Number(item.duration) : null;
      ...
      if (g === 'manga') totalDurationPerView = unitDuration * (Number(item.volumes) || 1);
      else if (g === 'anime' || g === 'drama') totalDurationPerView = unitDuration * (Number(item.episodes) || 1);

      const itemTotalMinutes = totalDurationPerView * views;
    ...
    const lifetimeStats = {
      ...
      genreLifetime: Object.entries(genreLifetime).map(([genre, mins]) => ({
        ...
        color: GENRE_COLORS[genre] || GENRE_COLORS.other,
        bgImage: (genreImages[genre] || [])[0] || null
    ...
    <div className="animate-in fade-in duration-700 space-y-8 pb-32">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6 mb-10">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-[11px] text-slate-500 font-black tracking-[0.3em] mt-4 flex items-center gap-3">
            統計・分析ダッシュボード
            <span className="w-12 h-px bg-slate-800" />
          </p>
        </div>

        <div className="inline-flex items-center gap-4 bg-emerald-500/10 px-6 py-3 rounded-2xl border border-emerald-500/20 shadow-lg backdrop-blur-md self-end sm:self-start">
          <span className="text-xs font-black text-slate-400 uppercase tracking-widest">総作品数</span>
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={stats.scoreBins}>
                ...
                <XAxis dataKey="range" axisLine={false} tickLine={false} tick={{ fill: 'rgba(255,255,255,0.5)', fontSize: 10 }} />
                <YAxis axisLine={false} tickLine={false} tick={{ fill: 'rgba(255,255,255,0.5)', fontSize: 10 }} />
                <RechartsTooltip cursor={{ fill: 'rgba(255,255,255,0.05)' }} contentStyle={{ backgroundColor: 'rgba(0,0,0,0.8)', border: 'none', borderRadius: '12px' }} />
                <Bar dataKey="count" fill="#10b981" radius={[4, 4, 0, 0]}>
                  {stats.scoreBins.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={index === 0 ? '#fbbf24' : index === 1 ? '#10b981' : '#3b82f6'} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
          <div className="mt-4 p-5 rounded-2xl bg-yellow-500/5 border border-yellow-500/10 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Trophy className="w-5 h-5 text-yellow-500" />
              <span className="text-xs font-black text-slate-300 uppercase tracking-widest">殿堂入り作品数</span>
            </div>
            <span className="text-2xl font-black text-white">{stats.hallOfFame.length}</span>
          </div>
        </div>
      </div>

      {/* Lifetime WATCHED */}
      <div className="bg-white/5 border border-white/10 rounded-3xl p-8 backdrop-blur-md overflow-hidden relative">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-10 relative z-10">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-2xl font-black text-white uppercase tracking-wider italic">LIFETIME WATCHED</h2>
            </div>
            <p className="text-xs text-slate-500 font-bold uppercase tracking-widest ml-12">累計視聴時間の解析</p>
          </div>
          
          <div className="flex flex-col items-end gap-2">
            <div className="text-5xl sm:text-7xl font-black text-white font-mono tracking-tighter flex items-baseline gap-2">
              <Counter value={stats.lifetimeStats.days} />
              <span className="text-xl text-yellow-500 uppercase italic">Days</span>
              <Counter value={stats.lifetimeStats.remainingHours} />
              <span className="text-xl text-yellow-500 uppercase italic">Hours</span>
            </div>
            <div className="px-4 py-1.5 bg-white/5 rounded-full border border-white/10">
              <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Total: {stats.lifetimeStats.totalHours.toLocaleString()} Hours</span>
            </div>
          </div>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 relative z-10">
          {stats.lifetimeStats.genreLifetime.map((item) => (
            <div key={item.id} className="group relative overflow-hidden rounded-2xl bg-black/40 border border-white/5 p-5 transition-all duration-500 hover:border-white/20">
              {item.bgImage && (
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8 backdrop-blur-md">
          ...
          <div className="flex overflow-x-auto gap-4 pb-4 no-scrollbar">
            {stats.hallOfFame.map((item) => (
              <div key={item.id} className="flex-shrink-0 w-44 group">
                <div className="aspect-[3/4] rounded-2xl overflow-hidden mb-3 relative border border-white/10">
                  {item.imageBase64 ? (
                  ...
                  <div className="absolute top-3 right-3 px-2 py-1 bg-black/60 backdrop-blur-md rounded-lg border border-yellow-500/30">
                    <span className="text-xs font-black text-yellow-400">{item.rating}</span>
                  </div>
                </div>
                <h3 className="text-sm font-black text-white line-clamp-1 group-hover:text-yellow-400 transition-colors">{item.title}</h3>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Footer / Quote */}
      <div className="mt-12 flex flex-col items-center gap-6 pt-12 border-t border-white/5">
        <div className="flex items-center gap-4 px-6 py-4 bg-white/5 rounded-2xl border border-white/10 backdrop-blur-sm max-w-2xl">
