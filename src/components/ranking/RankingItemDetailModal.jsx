        ...
        <div className="flex items-center justify-between p-6 border-b border-white/5 bg-black/60 backdrop-blur-md z-[100]">
          <div className="flex items-center gap-4">
             <div 
               ...
               className="flex items-center gap-3 bg-white/5 px-4 py-2 rounded-full border border-white/10 cursor-pointer hover:bg-white/10 transition-colors duration-150 active:scale-95"
             >
                <span className={`text-[10px] font-black uppercase tracking-[0.2em] transition-colors duration-150 ${isGlobalEditMode ? 'text-accent' : 'text-white'}`}>編集モード</span>
                <div className={`w-10 h-5 rounded-full p-1 flex items-center transition-colors duration-150 ${isGlobalEditMode ? 'bg-accent/30 border border-accent/50' : 'bg-white/10 border border-white/10'}`}>
                   <div className={`w-3 h-3 rounded-full transition-transform duration-200 ease-out ${isGlobalEditMode ? 'bg-accent translate-x-5' : 'bg-slate-600 translate-x-0'}`} />
               </div>
            </div>
            {hasChanges && (
              <button 
